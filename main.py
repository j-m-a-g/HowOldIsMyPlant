...
def main():
  ...
  prediction_result = model.predict(image_path_input.strip("\'\""), hosted=is_hosted_boolean, confidence=1,
                                    overlap=30).json()
  ...
  def Save_Predictions_As_Output_Image():
    ...
    try:
      model.predict(image_path_input.strip("\'\""), hosted=is_hosted_boolean, confidence=1, overlap=30).save(
        visualization_image_file_name, 8)
